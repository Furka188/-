...
def _extract_metrics(points: Dict[int, Tuple[float, float]]) -> Dict[str, float]:
    ...
    for left_idx, right_idx in symmetry_pairs:
        ...
        mirrored_l = abs(mid_x - lx)
        mirrored_r = abs(rx - mid_x)
        symmetry_errors.append(abs(mirrored_l - mirrored_r))
    ...
    # Normalized metrics for photo-to-photo comparability
    if ipd > 0:
    ...
    # Eye-line tilt (approximate roll angle)
...
def _top_distinguishing_points(
    ...
    shared = set(p1_map.keys()) & set(p2_map.keys())

    changes = []
    for idx in shared:
        d = _distance(p1_map[idx], p2_map[idx])
        changes.append({"landmark": idx, "delta_px": round(d, 4)})
...
def analyze_face(image_path: Path) -> AnalysisResult:
    ...
    missing = [i for i in required if i not in points]
    if missing:
        raise ValueError(f"Missing landmarks required for analysis: {missing}")

    metrics = _extract_metrics(points)
    quality = _image_quality(image)

    overlay = image.copy()
    # Dense white mesh points for premium dossier feel
...
def _build_project_zip() -> Path:
    archive_path = OUTPUT_DIR / "face-biometry-lab-premium.zip"
...
def compare_analyses(first: AnalysisResult, second: AnalysisResult) -> Dict[str, object]:
    ...
    for key in sorted(all_keys):
        ...
        metric_diff[key] = {
            ...
            "photo_2": round(v2, 4),
            "difference": delta,
            "difference_percent_from_photo_1": pct,
        }

    distinguishing_points = _top_distinguishing_points(first.landmarks, second.landmarks)
    return {
        "metric_differences": metric_diff,
        "top_distinguishing_landmarks": distinguishing_points,
    }
...
def analyze():
    ...
    if not photo or photo.filename == "":
        return jsonify({"error": "Attach a photo file"}), 400

    input_path = UPLOAD_DIR / f"{uuid.uuid4().hex}_{photo.filename}"
    photo.save(str(input_path))
...
def compare():
    ...
    if not photo_1 or not photo_2:
        return jsonify({"error": "Attach two photos"}), 400

    input_path_1 = UPLOAD_DIR / f"{uuid.uuid4().hex}_{photo_1.filename}"
    input_path_2 = UPLOAD_DIR / f"{uuid.uuid4().hex}_{photo_2.filename}"
    photo_1.save(str(input_path_1))
    photo_2.save(str(input_path_2))
